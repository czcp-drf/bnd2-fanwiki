...
- 대상 브랜치: `main`. 커밋 `34f512b`를 `deploy/main`에 푸시 완료했습니다.
## 2026-09-20 캐릭터 조직 참여 사건 표시

- 캐릭터 상세의 참여 사건에 직접 참여한 사건뿐 아니라, 캐릭터가 소속된 조직이 참여한 공개 사건도 포함했습니다.
- 직접 참여와 조직 참여가 같은 사건으로 겹치면 사건 카드를 하나로 합치며, 조직명과 조직 역할을 함께 표시합니다. 미확정 조직명은 조직 유형 라벨로 표시합니다.
...
- 검증: `npx tsc --noEmit`, `npm run build`, `git diff --check` 통과.
- 대상 브랜치: `main`. 커밋 `cad6871`을 `deploy/main`에 푸시 완료했습니다.

## 2026-09-21 연결된 불법 사업체의 조직 거점 표시

- 갱단과 연결된 불법 사업체를 공개 지도에서 별도 사업체 핀으로 구분하지 않고, 부모 갱단의 조직 거점과 같은 드롭 핀으로 표시하도록 수정했습니다.
- 해당 핀의 팝업에 갱단 정보와 불법 사업체명·위치 라벨·설명·위키 링크를 함께 표시합니다. 조직 상세에서 지도 포커스로 진입할 때도 연결된 위치와 팝업을 선택합니다.
- 기존 `gang_id`와 조직 좌표 데이터를 사용하며 별도 DB 마이그레이션은 필요하지 않습니다.
- 검증: `npx tsc --noEmit`, `npm run build`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 이번 변경과 문서 기록을 함께 커밋·푸시합니다.

## 2026-09-21 지도 핀 밝은 색상 대비 보완

- 밝은 색상의 조직·사업체·주요 장소 핀에 어두운 외곽선과 내부 하이라이트를 적용해 지도 배경 위에서도 식별되도록 수정했습니다.
...
## 2026-09-21 조직 대표 이모지

- 관리자 조직 추가·수정 화면에서 조직별 대표 이모지를 입력하거나 비울 수 있도록 추가했습니다. 비어 있으면 기존 로고·첫 글자 fallback이 유지됩니다.
- 조직 목록·상세·지도 팝업·사건 참여 조직·캐릭터/스트리머의 소속 조직 표시에서 대표 이모지를 우선 노출합니다.
...
- 재생 중에는 플레이어 영역 안에서만 컨트롤을 표시하고, 일시정지하면 마우스 위치와 관계없이 컨트롤을 유지합니다.
- 마지막으로 조절한 음량을 브라우저에 저장해 다른 BBS 영상과 기사에서도 공통으로 사용합니다. 저장값이 없으면 100%로 시작합니다.
- 검증: `npx tsc --noEmit`, `npm run build`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 커밋·푸시 상태는 완료 응답에서 확인합니다.

## 2026-09-22 BBS 영상 플레이어 후속 보완

- 관리자 본문 미리보기에서 영상 클릭 시 textarea `<label>` 포커스로 스크롤이 이동하던 문제를 수정했습니다.
- 영상 플레이어를 문단 안에서도 유효한 HTML 구조로 렌더링해 관리자 hydration 경고를 제거했습니다.
- 저장된 음량이 없을 때 0으로 초기화되던 처리를 수정해 기본 음량을 100%로 유지합니다.
- 클릭 직후 외곽선이 남지 않도록 마우스 포커스는 `focus-visible`, 누르는 순간은 `active` 상태로 구분했습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 커밋·푸시 상태는 완료 응답에서 확인합니다.

## 2026-09-22 BBS 영상 재생바 끝값 보정

- 영상 종료 시 현재 시간을 실제 duration으로 맞춰 재생바가 끝값에 도달하도록 보정했습니다.
- 재생바 이동 단위를 `step="any"`로 변경해 영상 길이가 소수점 단위여도 마지막 위치를 정확히 선택할 수 있도록 했습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 커밋·푸시 상태는 완료 응답에서 확인합니다.

## 2026-09-22 스트리머·캐릭터 상태 및 소속 표시 보완

- 스트리머 목록에서 조직 멤버십만 있는 캐릭터의 소속이 숨겨지던 조건을 수정했습니다. 조직이 없을 때는 등록된 직업을 표시합니다.
- 활동을 시작하지 않은 스트리머를 `활동 전`으로 표시하고, 사망·은퇴·휴식 캐릭터는 상태별 인원으로 구분해 표시합니다. 스트리머 상세의 상태 뱃지도 같은 문구로 맞췄습니다.
- 캐릭터 위키 목록 카드에 활동·사망·은퇴·휴식 상태 뱃지를 추가했습니다.
- 캐릭터 위키 조직 뱃지는 조직 색상을 배경·테두리 강조로 사용하고, 글자는 현재 라이트·다크 테마의 텍스트 색상을 사용해 대비를 보장합니다. DB 변경은 없습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 커밋·푸시 완료를 확인했습니다.

## 2026-09-24 캐릭터 상태 라벨 변경

- 화면에 표시되는 `retired` 상태 라벨을 `은퇴`에서 `활동 종료`로 변경했습니다.
- 캐릭터 위키·상세·스트리머·통합 검색·조직 멤버·관리자·봉스타그램 화면의 표시 문구를 통일했습니다.
- DB 상태값 `retired`와 필터·검증 로직은 유지했으며 별도 마이그레이션은 필요하지 않습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` → `deploy/main`. 커밋·푸시 상태는 완료 응답에서 확인합니다.

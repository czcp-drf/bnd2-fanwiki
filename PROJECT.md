...
- 사이트명 및 전역 설명 변경 (커밋·푸시 진행 중): 사용자·관리자 헤더와 메타데이터의 사이트명을 `봉누도 따라가기`로 통일하고, 기본·Open Graph·Twitter 설명도 `봉누도 따라가기`로 맞췄습니다. 제보 페이지와 홈 화면 표기도 새 사이트명에 맞춰 수정했습니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했으며 대상 브랜치는 `main`입니다. 원격 푸시 상태는 커밋 후 확인합니다.

...
- 사건 아카이브 클립 전환 및 시점명 빨간약 표시 (커밋·푸시 진행 중): 클립 플레이어에서 활성 iframe만 렌더링해 다른 클립을 선택하면 이전 클립 재생이 자동 중지되도록 수정했습니다. 플레이어·클립 목록·툴팁의 시점명을 빨간약 OFF에서는 캐릭터명, ON에서는 스트리머명으로 표시합니다. `npm run build`, `npx tsc --noEmit`, 변경 파일 ESLint, `git diff --check`를 통과했으며 대상 브랜치는 `main`입니다. 원격 푸시 상태는 커밋 후 확인합니다.

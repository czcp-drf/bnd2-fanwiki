...
- 조직 관리 삭제 버튼 표시 개선 (`fix/project-audit`): 일반 목록·편집 모드 모두 텍스트 삭제 버튼 표시, 삭제 확인/취소 유지, 관리 열 확장 및 좁은 화면 가로 스크롤 지원. 삭제 실패 메시지와 로딩 해제 처리 추가. 변경 파일 린트·TypeScript 검사 통과; 실제 조직 삭제는 수행하지 않았습니다.

...
- 홈 화면 개편 및 모바일 메뉴 레이어 수정 (`main`, 커밋·푸시 전): `봉누도 따라잡기` 컨셉에 맞춰 최근 사건·BBS 최신 기사·Bongstagram 최신 게시물을 홈의 핵심 영역으로 재배치하고 기존 위키 메뉴를 탐색 영역으로 정리했습니다. 홈·헤더 라이트/다크 테마 전환, 모바일 메뉴의 토글 제거, 지도보다 앞서는 Sheet 레이어, 소개 문구 개행을 반영했으며 변경 파일 ESLint·TypeScript·프로덕션 빌드·`git diff --check`를 통과했습니다. `deploy/main` 푸시를 진행합니다.

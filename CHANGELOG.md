...
- 지도 타일 bounds 경계 보정 (`main`, 커밋·푸시 진행): `MAP_TILE_BOUNDS`를 타일 세계의 정확한 끝점 대신 실제 게임맵 범위인 `MAP_MAX_BOUNDS`로 조정해 경계 밖 인접 타일이 요청 대상에 포함되는 문제를 줄였습니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

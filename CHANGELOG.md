...
- BBS JSON 일괄 가져오기 및 공개 목록 필터 복귀 보완 (`main`, 커밋·원격 푸시 예정): 어드민 기사 목록에서 인게임 BBS JSON의 `rows` 배열을 일괄 가져와 모든 기사를 비공개로 저장하고, 원본 ID로 재업로드 중복을 방지합니다. 본문 HTML은 Markdown으로 변환하고 첫 이미지를 대표 이미지로 지정하며, 외부 본문 이미지는 별도 첨부로 중복 저장하지 않습니다. 공개 기사 카드와 상세 페이지 사이에 현재 카테고리·기자·일차·정렬 조건을 전달해 상단 복귀 버튼에서도 필터를 유지합니다. 변경 파일 ESLint·TypeScript·프로덕션 빌드·`git diff --check`를 통과했으며 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

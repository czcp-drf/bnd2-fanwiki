...
- BBS 기사 상세 30일 캐시 및 사건 API 응답 축소 (`main`, 커밋·원격 푸시 예정): BBS 기사 상세 페이지·데이터 캐시와 상세 CDN 응답의 재검증 기간을 30일로 확대했습니다. 관리자 변경 시 캐시 무효화는 유지하며, 사건 목록 페이지와 API는 필요한 필드만 조회해 Origin Transfer를 줄였습니다. TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

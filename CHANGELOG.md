...
- 스트리머 검색 미정 처리 보정 (`main`, 커밋·푸시 진행): 빨간약 ON 상태의 캐릭터 목록 검색에서 미정 상태를 이름·별명 검색에만 제외하고 스트리머명 검색에는 적용하지 않도록 조건 순서를 조정했습니다. 미정 캐릭터가 연결된 스트리머도 스트리머명으로 정상 검색됩니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`이며 원격 `deploy/main` 푸시 결과를 확인해 기록할 예정입니다.

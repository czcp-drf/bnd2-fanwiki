...
- BBS 일차 필터·스크롤 복원 및 제보 관리 페이지네이션 (`main`, 커밋·원격 푸시 예정): BBS 운영일을 금요일 휴일 제외로 10월 4일까지 자동 계산하고, 공개 기사가 있는 일차만 선택 메뉴에 표시했습니다. 기사 상세 뒤로가기 복원은 유지하면서 홈의 BBS 전체보기 진입 시 저장된 스크롤 상태를 초기화합니다. 제보 관리에는 서버 검색과 10·20·30·40·50개 페이지네이션을 적용했습니다. TypeScript·관련 ESLint·프로덕션 빌드·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

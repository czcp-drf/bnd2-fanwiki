...
- 사건 참여 조직 표시·관련 사건 보완 (`main`, `34f512b`, 원격 `deploy/main` 푸시 완료): 사건 편집 화면과 공개 사건 상세의 조직 카테고리를 한글 라벨로 표시하고, 조직 상세 관련 사건에 `event_organizations`로 직접 연결된 공개 사건도 중복 없이 포함했습니다. `npx tsc --noEmit`, `npm run build`, `git diff --check`를 통과했습니다. Supabase MCP 원격 SQL 검증은 인증 scope 부족으로 실행하지 못했습니다.
...
- 2026-09-17: 사건 참여 인물에 포함되지 않은 클립 스트리머도 연결된 캐릭터명을 조회해 빨간약 OFF 상태의 클립 시점·라벨 매핑에 포함하도록 보완했습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` (`deploy` 원격으로 커밋·푸시)
...
- 2026-09-18: BBS 기사 상세 열람 상태를 브라우저 로컬 저장소에 기록하고, 목록에서 읽은 기사에 표시와 투명도 조정을 적용했습니다. 최대 1,000개까지 저장하며 서버 데이터에는 저장하지 않습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` (`deploy` 원격으로 커밋·푸시 완료)


- 2026-09-19: 스트리머·캐릭터 목록 검색에서 이름·별명·연결된 스트리머명의 공백을 무시하도록 개선해 `A B C`와 `ABC`를 동일하게 검색할 수 있도록 했습니다.
- 검증: `npx tsc --noEmit`, `git diff --check` 통과.
- 대상 브랜치: `main` (`deploy` 원격으로 커밋·푸시 완료)
...
## 2026-09-19 지도 SVG 색상 검증

- 공통 색상 검증 및 기본색 처리 추가. 조직·장소 관리자 저장에서 6자리 HEX만 허용하고 공개 지도·미니맵 SVG에도 검증을 적용합니다.
- 관리자 지도 표시도 동일 처리하며 거점·사업체 수정 요청의 필드를 명시해 색상 검증 우회를 방지합니다.
- 검증: 악성 문자열과 정상 색상, 관리자 저장 4개 함수, SVG 생성 5개 함수 검증 및 TypeScript·diff 검사 통과.
- 대상 main → deploy/main, 기록 시점 커밋·푸시 준비 상태. 푸시 결과는 완료 응답에서 안내합니다. 운영 배포는 미확인, DB 변경은 없습니다.

## 2026-09-19 관리자 로그인 반복 시도 제한

- migration 048 및 기본 스키마에 서비스 롤 전용 로그인 제한 RPC·테이블 추가. IP 해시별 15분간 5회 허용하며 대기 시간(초)을 반환합니다.
- 로그인은 비밀번호 검사 전 제한 RPC를 호출하고, 환경변수·IP·DB 확인 실패 시 인증을 거절합니다.
- 검증: PGlite 제한·만료·권한·재실행 검증, 로그인 6개 모의 시나리오, TypeScript·diff 검사 통과.
- migration 048 선적용 후 배포 필요. 운영 적용 및 배포 완료는 미확인입니다.
- 대상 main → deploy/main, 커밋·푸시 준비 상태에서 기록. 실제 결과는 완료 응답에서 안내합니다.

...
- 어드민 사건 공개·발생일 필터 (`main`, 커밋·푸시 진행): 사건 관리 목록에 공개·비공개 필터, 발생일자 범위, 발생일 최신순·오래된순 정렬을 추가하고, 선택한 조건의 전체 결과를 기준으로 건수와 페이지네이션이 동작하도록 수정했습니다. 기존 유형·검색·페이지 크기 조건과 함께 URL에 상태를 유지합니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`이며 원격 `deploy/main` 푸시 결과를 확인해 기록할 예정입니다.

...
- 사건 클립 빨간약 주의 오버레이 (`feat/event-archive`, 커밋·푸시 완료): 빨간약 OFF 상태로 사건 상세에 들어오면 iframe은 미리 로드하되 불투명한 주의 오버레이로 가리고, `클립 보기`를 눌러 해당 클립을 공개하도록 수정했습니다. 빨간약 ON 상태에서는 오버레이 없이 바로 표시되며, 사건 상세에서 빨간약을 켜도 즉시 공개됩니다. 클립 정보 바와 플레이리스트는 보호 상태와 관계없이 유지합니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다.
...
- 사건 클립 노출 보호 보완 (`main`, 커밋 대기): 사건 상세의 클립 플레이어를 진입 시점의 빨간약 상태에 따라 열림·닫힘으로 초기화했습니다. 닫힌 상태에서도 iframe은 미리 로드하고 플레이리스트는 표시해 클립과 시점을 선택할 수 있으며, `클립 보기`를 직접 눌러야 영상이 노출됩니다. 빨간약 토글만으로 현재 플레이어가 갑자기 열리거나 닫히지 않도록 했습니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`이며 원격 푸시 전입니다.

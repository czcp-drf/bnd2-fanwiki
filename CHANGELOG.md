...
- 어드민 목록 페이지네이션 확대 및 캐릭터 스트리머 검색 (`main`, 커밋·원격 푸시 예정): 사건·스트리머·관계·IP 차단·캐릭터 관리 페이지를 서버 페이지네이션으로 전환하고 페이지당 10·20·30·40·50개 선택을 추가했습니다. 목록 페이지 이동 상태를 URL에 유지하며, 캐릭터 추가 폼의 스트리머 선택 메뉴에는 검색 기능을 적용했습니다. TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

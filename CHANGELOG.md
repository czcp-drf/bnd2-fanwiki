...
- BBS 본문 이미지 확대 및 대표 이미지 비율 보완 (`main`, 커밋·원격 푸시 예정): 본문 이미지를 클릭하면 확대 모달을 열 수 있도록 추가하고, 상세 대표 이미지를 `1200×900` 기준의 `4:3` 비율로 표시했습니다. 목록 카드 비율은 기존 `16:9`을 유지했습니다. TypeScript·ESLint·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

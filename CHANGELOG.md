...
- 스트리머 카드 헤더 중복 정보 제거 (`main`, 커밋·푸시 예정): 빨간약 ON일 때 카드 헤더에 반복 표시되던 활동 캐릭터명과 소속을 제거하고, 카드 본문의 캐릭터 정보만 유지했습니다. 스트리머 페이지의 카드 제목인 스트리머명은 유지합니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다.

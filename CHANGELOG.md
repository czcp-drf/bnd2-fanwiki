...
- 스트리머 검색 미정 필터 보정 (`main`, 커밋·푸시 진행): 빨간약 ON 상태의 스트리머 검색에서도 검색어가 있을 때 `is_name_pending` 캐릭터를 항상 제외하도록 수정했습니다. 실제 이름으로 입력한 `미정`과 `신미정`은 계속 검색할 수 있습니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`이며 원격 `deploy/main` 푸시 결과를 확인해 기록할 예정입니다.

...
- 어드민 BBS 전체 반응 수 정렬 (`main`, 커밋·원격 푸시 예정): 좋아요·싫어요 정렬을 현재 페이지 내부 정렬에서 전체 필터 결과 기준 정렬로 변경한 뒤 페이지를 나누도록 수정했습니다. 반응 필터와 검색·카테고리·공개 상태·담당기자 조건도 전체 결과에 적용합니다. TypeScript·관련 ESLint·`git diff --check`를 통과했습니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

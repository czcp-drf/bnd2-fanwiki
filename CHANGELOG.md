...
- BBS 공개 목록 일차 필터·정렬·무한 스크롤 및 관리자 입력 버튼 보완 (`main`, 커밋·원격 푸시 예정): 승인일시 기준 일차 필터를 추가하고, 일차 선택 모달은 외부 클릭으로 닫히며 작은 화면에서 버튼 왼쪽 기준으로 뷰포트 안쪽에 자동 배치됩니다. 최신순·오래된순 정렬을 기자 필터에 추가하고 URL·카테고리·일차·무한 스크롤 요청에 상태를 유지했습니다. 공개 기사 목록은 최초 12건 조회 후 하단 도달 시 다음 페이지를 추가하며 필터 변경 시 목록을 초기화합니다. 관리자 기사 등록·수정·취소 버튼을 입력 폼 상단에도 배치했습니다. 변경 파일 ESLint·TypeScript·`git diff --check`를 통과했으며 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

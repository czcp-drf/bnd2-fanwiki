...
- 관리자 BBS JSON 입력기 Markdown 변환 보완 (`main`, 커밋·원격 푸시 예정): 관리자 JSON 가져오기에서 제목·강조·링크·이미지·줄바꿈과 인용문·구분선·코드·표를 안정적으로 Markdown으로 변환하고, 일반 텍스트의 Markdown 특수문자 오작동과 불필요한 HTML 태그 유입을 방지했습니다. TypeScript·관련 ESLint·`git diff --check`를 통과했습니다. 공개 HTML 변환기 파일은 이번 커밋에서 제외합니다. 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

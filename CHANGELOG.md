...
- BBS 기사 상세 연속 이동 및 대표 이미지 로딩 보완 (`main`, 커밋·원격 푸시 예정): 기사 상세 하단에 이전 기사·다음 기사 버튼을 추가하고 현재 카테고리·기자·일차·정렬 조건을 유지한 채 이동하도록 했습니다. 상세 대표 이미지에는 `loading="eager"`를 적용했습니다. 목록 스크롤 저장·복원 방식은 제거했습니다. 변경 파일 ESLint·TypeScript·프로덕션 빌드·`git diff --check`를 통과했으며 대상 브랜치는 `main`, 원격은 `deploy/main`입니다.

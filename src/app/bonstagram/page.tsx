...
function StoryBubble({
  ...
  return (
    <div className="flex w-[4.5rem] shrink-0 flex-col items-center gap-1.5">
      <div className="relative rounded-full bg-gradient-to-tr p-[2px] from-zinc-800 to-zinc-700">
        {tone && <div className={`absolute inset-0 rounded-full bg-gradient-to-tr ${tone}`} />}
        <div className="relative flex h-[4.25rem] w-[4.25rem] items-center justify-center rounded-full border-2 border-zinc-950 bg-zinc-800 text-xl font-bold text-zinc-200">
          {mark}
        </div>
        {mine && (
          <span className="absolute bottom-0 right-0 flex h-5 w-5 items-center justify-center rounded-full border-2 border-zinc-950 bg-sky-500 text-white">
...
function BottomNav() {
  return (
    <nav className="flex items-center justify-around border-t border-zinc-800 bg-zinc-950/95 px-3 py-3 backdrop-blur" aria-label="Bonstagram 메뉴">
    ...
    <div className="bonstagram-font min-h-[calc(100vh-3.5rem)] bg-zinc-950">
      <div className="mx-auto min-h-[calc(100vh-3.5rem)] w-full max-w-[540px] border-x border-zinc-900 bg-zinc-950">
        <header className="flex h-16 items-center justify-between border-b border-zinc-800 px-5">
          <Link href="/bonstagram" className="inline-block origin-left scale-x-105 text-2xl font-normal leading-none tracking-tight text-white">
            Bonstagram
          </Link>
        ...
        <main>
          <article className="border-b border-zinc-800">
            ...
            <div className="flex aspect-[4/5] items-center justify-center bg-gradient-to-br from-zinc-900 via-zinc-950 to-fuchsia-950/20">
              <div className="flex max-w-[16rem] flex-col items-center gap-4 px-6 text-center">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-xs leading-5 text-zinc-500">캐릭터별 Bonstagram 프로필과 게시물 데이터가 연결되면 이 영역에 사진이 표시됩니다.</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 px-4 py-3">
              ...
              <p className="text-sm text-zinc-300">봉누도2의 소식과 일상을 Bonstagram에서 만나보세요.</p>
            </div>
          </article>
        </main>

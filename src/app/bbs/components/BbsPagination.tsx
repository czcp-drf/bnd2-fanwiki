...
function getPageHref(page: number, category: BbsCategory, reporterIds: string[], day?: BbsDayKey) {
...
export default function BbsPagination({ currentPage, totalPages, category, reporterIds, day }: { currentPage: number; totalPages: number; category: BbsCategory; reporterIds: string[]; day?: BbsDayKey }) {
  ...
  return (
    <nav aria-label="BBS 기사 페이지" className="flex items-center justify-center gap-1 pt-3">
      <Link
        href={getPageHref(Math.max(1, currentPage - 1), category, reporterIds, day)}
        aria-label="이전 페이지"
      ...
      {pages.map((page) => (
        <Link
          key={page}
          href={getPageHref(page, category, reporterIds, day)}
      ...
      <Link
        href={getPageHref(Math.min(totalPages, currentPage + 1), category, reporterIds, day)}
        aria-label="다음 페이지"

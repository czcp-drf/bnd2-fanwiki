...
export default function BbsHeader({ activeCategory, activeReporterIds, reporters }: Props) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[var(--bbs-border)] bg-[var(--bbs-surface)] px-5 py-5 sm:px-7 md:static md:rounded-t-3xl md:px-8 md:py-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => { setSelectedReporterIds(activeReporterIds); setReporterMenuOpen(false); setFilterOpen((open) => !open) }}
      ...
      {filterOpen && (
        <div className="absolute right-5 top-[4.75rem] z-40 w-[min(calc(100vw-2.5rem),20rem)] rounded-2xl border border-[var(--bbs-border)] bg-[var(--bbs-card)] p-4 shadow-xl sm:right-7 md:right-8" role="dialog" aria-label="기사 필터">
          ...
          <div className="relative mt-3 text-xs text-[var(--bbs-subtle-text)]">
            ...
            <button type="button" aria-haspopup="listbox" aria-expanded={reporterMenuOpen} onClick={() => setReporterMenuOpen((open) => !open)} className="mt-1.5 flex w-full cursor-pointer items-center justify-between gap-2 rounded-lg border border-[var(--bbs-border)] bg-[var(--bbs-muted)] px-3 py-2.5 text-left text-sm text-[var(--bbs-text)] transition-colors hover:border-[#e14b32]/60 focus:border-[#e14b32] focus:outline-none">
              <span className="truncate">{selectedReporterIds.length === 0 ? '모든 기자' : selectedReporterIds.length === 1 ? reporters.find((reporter) => reporter.id === selectedReporterIds[0])?.name ?? '선택한 기자 1명' : `선택한 기자 ${selectedReporterIds.length}명`}</span>

...
export default function BbsArticleVisual({ article, detail = false, compact = false }: { article: BbsArticle; detail?: boolean; compact?: boolean }) {
  ...
  return (
    <div className={`${aspectClass} isolate overflow-hidden bg-gradient-to-br ${toneStyles[article.categoryKey]}`}>
      {imageUrl ? (
        detail ? <BbsZoomableImage src={imageUrl} alt="기사 대표 이미지 확대" intrinsic previewClassName="max-w-full rounded-xl" sizes="(max-width: 768px) 100vw, 768px" loading="eager" /> : <AppImage src={imageUrl} alt="" fill sizes="(max-width: 768px) 100vw, 576px" className="object-cover" />
      ) : (

...
type Props = { initialEvents: Event[]; type: string; day?: BbsDayKey; initialHasMore: boolean; total: number }
...
export default function EventArchiveFeed({ initialEvents, type, day, initialHasMore, total }: Props) {
  const sentinelRef = useRef<HTMLDivElement>(null)
  const restoreTargetRef = useRef<number | null>(null)
  const restoreAttemptsRef = useRef(0)

  const query = useInfiniteQuery({
    queryKey: ['events', type || null, day || null],
    ...
    <>
      {events.length === 0 ? (
        ...
        <div className="space-y-6">
          {!type && <FeaturedEvent event={events[0]} type={type} day={day} />}
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {visibleEvents.map((event) => <EventCard key={event.id} event={event} type={type} day={day} />)}
          </div>
...
function eventHref(id: string, type: string, day?: BbsDayKey) {
  const params = new URLSearchParams()
  if (type) params.set('type', type)
  if (day) params.set('day', day)
  const query = params.toString()
  return `/events/${id}${query ? `?${query}` : ''}`
}

function FeaturedEvent({ event, type, day }: { event: Event; type: string; day?: BbsDayKey }) {
  return (
    <Link href={eventHref(event.id, type, day)} onClick={rememberEventScrollPosition} className={`group relative flex min-h-[260px] flex-col justify-end overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 transition-colors hover:border-amber-400/30 ${event.thumbnail_url ? 'events-featured-card' : ''}`}>

...
function FeaturedEvent({ event }: { event: Event }) {
  return (
    <Link href={`/events/${event.id}`} className={`group relative flex min-h-[260px] flex-col justify-end overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 transition-colors hover:border-amber-400/30 ${event.thumbnail_url ? 'events-featured-card' : ''}`}>

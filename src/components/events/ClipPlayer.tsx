...
export default function ClipPlayer({
  ...
  const [isExpanded, setIsExpanded] = useState(isRedPill)
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="w-full space-y-0 rounded-xl border border-zinc-800 overflow-hidden">
        {/* 영상 영역 — 닫혀 있어도 iframe은 미리 로드하고 화면에만 숨김 */}
        <div
          className="relative w-full overflow-hidden"
          style={isExpanded ? { paddingBottom: '56.25%' } : { height: 1 }}
        >
          {clips.map((clip) => {
            ...
            return (
              <iframe
                ...
                style={{ border: 'none', visibility: isActive && isExpanded ? 'visible' : 'hidden' }}
              />
            )
          })}
        </div>
        ...
        {isExpanded ? (
          ...
          <div className="flex flex-col gap-3 bg-zinc-900 px-4 py-4 sm:px-5">
            <div className="flex items-center gap-3 min-w-0">
              <EyeOff size={20} className="shrink-0 text-zinc-600" />
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-zinc-500 mt-0.5 truncate">
                  {activePerspective ? `${activePerspective} 시점` : '관련 클립'}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsExpanded(true)}
                className="shrink-0 cursor-pointer rounded-lg border border-zinc-700 px-3 py-1.5 text-xs font-medium text-zinc-300 hover:border-zinc-500 hover:bg-zinc-800 hover:text-white"
              >
                클립 보기
              </button>
            </div>
